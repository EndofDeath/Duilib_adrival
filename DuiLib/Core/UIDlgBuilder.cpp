...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        // Process attributes
        if( node.HasAttributes() ) {
			//Init attributes from AttributeList
			if (pManager)
			{
				LPCTSTR pAttributesList = node.GetAttributeValue(_T("default"));
				if (0 != _tcsicmp(pAttributesList,_T("")))
				{
					LPCTSTR pAttributes = pManager->GetDefaultAttributeList(pAttributesList);
					if (pAttributes)
						pControl->SetAttributeList(pAttributes);
				}
			}
			// Set ordinary attributes
            int nAttributes = node.GetAttributeCount();
