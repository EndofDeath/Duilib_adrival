...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
	...
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bDrawCaret(true), m_bInited(false), m_dwReadonlyBkcolor(0xfff5f5f5), m_hImc(NULL)
{
...
{
	if (NULL != m_hImc)
		ImmReleaseContext(GetManager()->GetPaintWindow(), m_hImc);
    if( m_pTwh ) {
        m_pTwh->Release();
...
void CRichEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if (_tcscmp(pstrName, _T("readonlybkcolor")) == 0)
	{
		if (*pstrValue == _T('#')) pstrValue = ::CharNext(pstrValue);
		LPTSTR pstr = NULL;
		DWORD clrColor = _tcstoul(pstrValue, &pstr, 16);
		SetReadonlyBkColor(clrColor);
	}
...
LRESULT CRichEditUI::MessageHandler(UINT uMsg, WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	...
	{
		if (NULL == m_hImc)
			m_hImc = ImmGetContext(GetManager()->GetPaintWindow());
		if (IsReadOnly())
		{	
			ImmAssociateContext(GetManager()->GetPaintWindow(), NULL);
		}
		else
		{ 
			ImmAssociateContext(GetManager()->GetPaintWindow(), m_hImc);
		}
	}
...
void CRichEditUI::SetReadonlyBkColor(DWORD dwBkColor)
{
	m_dwReadonlyBkcolor = dwBkColor;
}

DWORD CRichEditUI::GetReadonlyBkColor() const
{
	return m_dwReadonlyBkcolor;
}
...
void CRichEditUI::PaintBkColor(HDC hDC)
{
	if (IsReadOnly())
	{
		CRenderEngine::DrawColor(hDC, m_rcPaint, GetAdjustColor(m_dwReadonlyBkcolor));
	}
	else
	{
