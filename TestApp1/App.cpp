...
class CFrameWindowWnd : public CWindowWnd, public INotifyUI, public CDwm, public CDPI, public IDialogBuilderCallback
{
public:
	CFrameWindowWnd() : m_pWndShadow(NULL), m_pLnk1(nullptr){};
	...
	{
		if (_tcsicmp(pstrClass, _T("LnkButton")) == 0)
			return new LnkButton();
		if (_tcsicmp(pstrClass, UI_DRAGTEXT) == 0)
			return new CDragTextUI();
		return NULL;
	}

    void Init() { 
		m_pLnk1 = static_cast<LnkButton*> (m_pm.FindControl(_T("testbtn")));
    ...
    void Notify(TNotifyUI& msg)
    {
        if( msg.sType == _T("windowinit") ) OnPrepare();
			...
			else if (msg.pSender->GetName() == _T("openfolderbtn"))
			{
				::ShellExecute(NULL, _T("open"), _T("D:\\Project"), NULL, NULL, SW_SHOWNORMAL);
			}
			else if (typeid(*msg.pSender) == typeid(LnkButton))
			{
				LnkButton* p = static_cast<LnkButton*>(msg.pSender);
				::ShellExecute(NULL, _T("open"), p->GetLnk(), NULL, NULL, SW_SHOWNORMAL);
			}
			else if (msg.pSender->GetName() == _T("btn1"))
			{
				::ShellExecute(NULL, _T("open"), _T("D:\\"), NULL, NULL, SW_SHOWNORMAL);
			}
        }
		else if (msg.sType == _T("selectchanged"))
		{
			...
			else if (name == _T("select2"))
				pControl->SelectItem(1);
			else if (name == _T("select3"))
				pControl->SelectItem(2);
			else if (name == _T("select4"))
				pControl->SelectItem(3);
			else if (name == _T("select5"))
    ...
    LRESULT HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        if( uMsg == WM_CREATE ) {
			...
			CControlUI* pRoot = builder.Create(_T("test1.xml"), (UINT)0, this, &m_pm);
